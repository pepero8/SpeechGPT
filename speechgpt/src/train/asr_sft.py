...
def train():
    ...
    def compute_metrics(eval_preds) -> Dict:
        ...
        # Extract transcription predictions (text after '>')
        # transcription_preds = []
        # for pred in decoded_preds:
        #     parts = pred.split(">")
        #     if len(parts) > 1:
        #         # Take the text after '>', strip whitespace
        #         transcription = parts[1].strip()
        #         transcription_preds.append(transcription)
        #     else:
        #         # If no '>' found, use empty string or the full text
        #         transcription_preds.append("")
        ...
        bertscore_results = bertscore.compute(
            predictions=decoded_preds, references=[[label] for label in decoded_labels], lang="en"
        )
